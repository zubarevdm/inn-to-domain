...
4.1. Признак "найден_по_запросу_с_ИНН" означает, что домен появился в выдаче по запросу с ИНН этой организации. Это сильный довод за.
4.2. ИНН организации может стоять и на чужом сайте: у партнёра, у дочерней компании, на сайте филиала, в карточке агрегатора. Смотри на контекст реквизита и на то, чей это сайт по содержанию.
        ...
        payload.append(
            {
                ...
                "найден_по_запросу_с_ИНН": candidate.from_inn_query,
                "домен_зарегистрирован": candidate.domain_created,
                "страницы": evidence,
            }
        )
